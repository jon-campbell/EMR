...
function level_idle()
	...
	if (danger_compass) then
		...
		if ((e <= min_left) and (not task_complete)) then
			Players[0]:print('Head back up to the rampart...');
			Players[0]:print('You are ready for your next task.');
			Players[0]:play_sound(233, 1);
			task_complete = true;
		end

		Players[0].overlays[0].text = "Enemies Left: " .. e
	...
	if (loc.index ~= last_poly) then
		...
		if ((loc.index == cave_entrance.index) and (not caves_open)) then
			Players[0]:print('Come back after learning the Scroll of Deliverance');
		end
		...
		if (poly_type == "hill") then
			...
			elseif freed then
				...
				Players[0]:print('Enemies left in the area: '..e);
				Players[0]:print('Enter the rampart pod for more information');
			else
				Players[0]:print('Come back after freeing the prisoners held in the dungeon');
			end
		end
	end
end
