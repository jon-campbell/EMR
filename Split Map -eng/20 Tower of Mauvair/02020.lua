...
function level_idle()
	...
	if (player_poly ~= last_poly) then
		...
		if (not clear_teleport) then
			clear_teleport = true;
			Players[0]:print('Sequencing complete');
		end
		...
		if ((player_poly == teleport_poly) and Lights[teleport_light].active and (not open_wait)) then
			teleport_platform.active = true;
		end
	...
	if ((player_poly == teleport_poly) and
			Lights[teleport_light].active and
			(not start_teleport) and
			clear_teleport and
			(teleport_platform.ceiling_height < 0.2)) then
		Players[0]:print('Charging Teleport...');
		start_teleport = true;
		prev_count = -1;
	end
